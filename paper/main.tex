...
\label{tab:rules}
  \begin{tabular}{ll|ll|ll}
    ...
    {\tt < ==> >} & {\tt :[a]<:[b] ==> :[a]>:[b]}  &  {\tt  else ==>}  & {\tt  else ==>} & {\tt synchronized ==>} &  {\tt synchronized ==>} \\
    {\tt < ==> ==} & {\tt :[a]<:[b] ==> :[a]==:[b]} &  {\tt if (\textbackslash(.*\textbackslash)) ==> if (!\textbackslash 1)} &  {\tt if (:[cond]) ==> if (!(:[cond]))} & {\tt (\^{}\textbackslash s*)(\textbackslash S+.*\textbackslash n) ==> \textbackslash 1pass} & {\tt
                                                                          :[ s]:[expr]:[lf~[\textbackslash n]] ==> :[s]pass:[lf]} \\
{\tt < ==> <=}  & {\tt :[a]<:[b] ==> :[a]<=:[b]} & {\tt if (\textbackslash(.*\textbackslash)) ==> if
                 (1==1)} & {\tt if (:[cond]) ==> if (1==1)} &  {\tt any\_of ==> all\_of} & {\tt any\_of ==> all\_of} \\
\hline
\end{tabular}
\end{figure*}
