...
\item Contiguous whitespace (e.g., spaces and newline) match contiguous whitespace in the source code. That is, match templates are sensitive to the \emph{presence} of whitespace, but \emph{not} the exact layout of spaces (i.e., the number of spaces may not correspond exactly between template and source code).
...
\begin{table*}[hbtp]
    \centering
    \caption{Overview of Mutation Testing Tools. Our tool has significantly fewer lines of code than the single-language tools.}
...
\caption{Java (Our Implementation vs. PIT vs. LittleDarwin).
  Determining if PIT bytecode mutants are equivalent is extremely
  difficult to do with reasonable accuracy. LittleDarwin produces no equivalent mutants.}
