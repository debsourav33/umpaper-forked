...
\begin{figure*}[hbtp]
\centering
\caption{Regular Expression Rule Examples \clg{Comby, or PPC, you think?}}
\label{tab:rules}
  \begin{tabular}{ll|ll|ll}
    \hline
    \mc{2}{c|}{Universal}                          & \mc{2}{c|}{C-Like}      & \mc{2}{c|}{Language-Specific} \\
    Regexp & Comby                                 & Regexp & Comby & Regexp & Comby     \\
    {\lstinline|< ==> >|} & {\lstinline|:[a]<:[b] ==> :[a]>:[b]|}  &  {\lstinline|else ==>|}  & {\lstinline|else ==>|} & {\lstinline|synchronized ==>|} &  {\lstinline|synchronized ==>|} \\
   {\lstinline|< ==> ==|} & {\lstinline|:[a]<:[b] ==> :[a]==:[b]|} &  {\lstinline|if (\(.*\)) ==> if (!\1)|} &  {\lstinline|if (:[cond]) ==> if (!(:[cond]))|} & {\lstinline|(\^{}\s*)(\S+.*\n) ==> \1pass|} & {\lstinline|:[ s]:[expr]:[lf~[\n]] ==> :[s]pass:[lf]|} \\
{\lstinline|< ==> <=|}  & {\lstinline|:[a]<:[b] ==> :[a]<=:[b]|} & {\lstinline|if (\(.*\)) ==> if (1==1)|} & {\lstinline|if (:[cond]) ==> if (1==1)|} &  {\lstinline|any_of ==> all_of|} & {\lstinline|any_of ==> all_of|} \\
\hline
\end{tabular}
\end{figure*}
...
\caption{Java (Our Implementation vs. PIT vs. LittleDarwin).
  Determining it PIT bytecode mutants are equivalent is extremely
  difficult to do with reasonable accuracy, so we did not attempt it.  LittleDarwin produces no equivalent mutants.}
