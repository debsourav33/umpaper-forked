...
\caption{Regex vs. Comby for Python.   LOC describes the project files considered. Muts. is total number of mutants generated for those files. \clg{FIXME: what's an invalid mutant for python?}  We can use TCE to check for redundant Python mutants, shown under ``Red.''}
...
\begin{table}[htbp]
\centering
\caption{Java (Our Implementation vs. PIT vs. LittleDarwin). \clg{FIXME: explain why no eqv for PIT.} LittleDarwin produces no equivalent mutants.\clg{right?}}
